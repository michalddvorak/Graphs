...
{
public:
	inline bool operator ()(const pair<double, coord> &a, const pair<double, coord> &b)
	{
...
bool Graph::Astar(coord start, coord end, CharMatrix &space, list<coord> &path, size_t &nodes_expanded)
{
	...
	priority_queue<pair<double, coord>, vector<pair<double, coord>>, Comparer> queue;

	for (auto &it:vertices)
		dist[it.first] = std::numeric_limits<double>::infinity();

	dist[start] = 0;
	queue.push(make_pair(sqrdiffer(start, end), start));
	while (!queue.empty())
	{
		auto x       = queue.top().second;
		queue.pop();//extract element with least distance
		//If it is the end, reconstruct path and return
		if (x == end)
		{
			path = reconstruct_path(prev, x);
		...
		for (auto &it : vertices[x]->adjacent)
		{
			...
			double     d        = dist[x] + it.first; //d = distance to x + distance to its neighbour;
				...
				//update it, if curdata was unvisited, this always occurs
			{
				...
				queue.push(make_pair(d + sqrdiffer(curdata, end), curdata)); //update curdata in distances
...
bool Graph::Dijkstra(coord start, coord end, CharMatrix &space, list<coord> &path, size_t &nodes_expanded)
{
	...
	while (!queue.empty())
	{
		...
		for (auto &it : vertices[x]->adjacent)
		{
			...
			{
				dist[curdata] = d; // insert this better result
				queue.push(make_pair(d, curdata)); //update curdata in distances
				prev[curdata] = x; //for path
